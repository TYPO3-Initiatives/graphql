    ...
    public static function validate(ResolveInfo $info, ?TreeNode $expression, string ...$types)
    {
        ...
        foreach ($expression->getChildren() as $item) {
            ...
            foreach ($constraint as $type) {
                ...
                if (
                    count($path->getChildren()) > 1 ||
                    Type::isCompositeType($schema->getType($type)->getField($field)->getType())
                ) {
                    throw new NotSupportedException(
                        sprintf('Composite field "%s" is not supported in order clause', $field),
                        1560598442
                    );
                }

                if (
                    $meta instanceof EntityDefinition && $meta->getName() !== $type ||
                    $meta instanceof PropertyDefinition && !$meta->hasActiveRelationTo($type)
                ) {
                    throw new SchemaException(
